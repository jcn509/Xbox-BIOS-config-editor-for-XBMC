...
class Flubber(AbstractIndBiosTab):
    ...
    def _create_controls(self):
        ...
        self._place_and_link(
            "GLOWCOLOR",
            controls.ColourPicker("Glow colour"),
            0, 3
        )
        self._place_label("In/out glow colour", 0, 4)
        self._place_and_link(
            "IOGLOWCOLOR",
            controls.ColourPicker("Intro/outro glow colour"),
            0, 5
        )
        ...
        self._place_and_link(
            "FOGON",
            controls.RadioButton("Enable fog"),
            1, 0
        )
        ...
        self._place_and_link(
            "SHOWFLUB",
            controls.RadioButton("Show flubber"),
            2, 0
        )
        self._place_and_link(
            "NOSOUND",
            controls.RadioButton("Disable Sound"),
            2, 1
        )
        self._place_and_link(
            "480P",
            controls.RadioButton("480P"),
            2, 2
        )
        self._place_and_link(
            "FASTANI",
            controls.RadioButton("Fast Animation"),
            2, 3
        )
        self._place_label("Camera view", 2, 4)
        self._place_and_link(
            "CAMERAVIEW",
            controls.SelectBox(["Random"] + [str(x) for x in range(0,16)], "Random", "Camera view"),
            2, 5,
            custom_control_to_config_converter = lambda value: value.replace("Random", "-1"),
            custom_config_to_control_converter = lambda value: str(value).replace("-1","Random")
        )
        self._place_and_link(
            "IFILTER",
        ...
        self._place_and_link(
            "BLOBTHROB",
            controls.RadioButton("Blob throb"),
            4, 3
        )
        self._place_and_link(
            "SLOWMOBLOB",
            controls.RadioButton("Slow Down Blob"),
            4, 4
        )
        self._place_and_link(
            "SPIKEYBLOB",
            controls.RadioButton("Spikey blob"),
            4, 5
        )
        self._place_label("Blob colour", 5, 0)
        self._place_and_link(
            "BLOBCOLOR",
            controls.ColourPicker("Blob olour"),
            5, 1
        )
        self._place_label("Radi of the blob", 5, 2)
        self._place_and_link(
            "BLOBRADI",
            controls.FakeSlider(min_value = 0, max_value = 100, picker_title = "Radi of the blob"),
            5, 3,
            columnspan = 2
        )
