...
class ColourPicker(AbstractControl, pyxbmct.Group):
    ...
    def __new__(cls, window_title = "Choose Colour", alpha_selector = False, default_colour = "0xFFFFFFFF", *args, **kwargs):         
        return super(ColourPicker, cls).__new__(cls, 1, 2, *args, **kwargs)

    def __init__(self, window_title = "Choose Colour", alpha_selector = False, default_colour = "0xFFFFFFFF", *args, **kwargs):
    ...
    def pick_colour(self):
        ...
        colour_picker = windows.ColourPicker(self._window_title, self._alpha_selector, self._current_colour, self.set_value)
        colour_picker.doModal()
        # Destroy the instance explicitly because
        # underlying xbmcgui classes are not garbage-collected on exit.
        del colour_picker

    def _colour_square_placed(self, window, row, column, rowspan, columnspan, pad_x, pad_y):
        ...
        square_side = min(self._colour_square.getWidth(),
                           self._colour_square.getHeight())

        button_x, button_y = self._button.getPosition()
        button_width = self._button.getWidth()
        
        new_square_x = button_x + button_width - \
                       ((square_side + square_width)/2 + pad_x)
