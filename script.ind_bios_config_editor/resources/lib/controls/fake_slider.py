...
class FakeSlider(AbstractControl, pyxbmct.Group):
    ...
    def __new__(cls, min_value=0, max_value=255, default_value=None, step=1, click_to_type_value=True,
                picker_title="Enter Number", show_value_label=True, value_label_width=1, total_width=4,
                textureback=None, texture=None, texturefocus=None, *args, **kwargs):
        return super(FakeSlider, cls).__new__(cls, 1, 1, *args, **kwargs)

    def __init__(self, min_value=0, max_value=255, default_value=None, step=1, click_to_type_value=True,
                 picker_title="Enter Number", show_value_label=True, value_label_width=1, total_width=4,
                 textureback=None, texture=None, texturefocus=None, *args, **kwargs):
        ...
        self._outline_texture = textureback if textureback else os.path.join(pyxbmct.skin.images, 'Slider',
                                                                             'osd_slider_bg.png')
        self._nib_texture_not_focussed = texture if texture else os.path.join(pyxbmct.skin.images, 'Slider',
                                                                              'osd_slider_nibNF.png')
        self._nib_texture_focussed = texturefocus if texturefocus else os.path.join(pyxbmct.skin.images, 'Slider',
                                                                                    'osd_slider_nib.png')
    ...
    def set_value(self, value, trigger_callback=True):
        ...
        self._value = value
        nib_x = int(round(self._nib_min_x + (float(value - self._min_value) * self._nib_x_unit)))
    ...
    def _placedCallback(self, window, *args, **kwargs):
        ...
        if self._show_value_label:
            ...
            self.placeControl(self._value_label, 0, label_column, columnspan=self._value_label_width, pad_x=10, pad_y=0)
        else:
            slider_width = self._total_width

        self._window = window
        width = int(round(float(self.getWidth()) * (float(slider_width) / float(self._total_width))))
        ...
        self._nib_x_unit = float(width - nib_size) / float(self._max_value - self._min_value)

        self._nib = pyxbmct.Button("", focusTexture=self._nib_texture_not_focussed,
